...
export class CustomLoopbackClient implements ILoopbackClient {
  port = 0; // default port, which will be set to a random available port
  private server: http.Server | undefined;

  private constructor(port = 0) {
    this.port = port;
  }
  ...
  static parseQueryString(queryString: string): string {
    ...
    if (queryIndex2 > -1) {
      return queryString.slice(Math.max(0, queryIndex2 + 2));
    } else if (queryIndex1 > -1) {
      return queryString.slice(Math.max(0, queryIndex1 + 1));
    }
    return '';
  }
